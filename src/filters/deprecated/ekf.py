...
class EKF(FilterBuilder):
    ...
    def build_correct(self) -> FilterCorrect:
        def correct(
            ...
            # jax.debug.print("y={y}, P={P}, K={K}", y=y_delta, P=P, K=K)

            x_corrected = x + (K @ y_delta).reshape(*x.shape)  # [1, N, D]

            # Joseph form update
            A = jnp.eye(P.shape[-1]) - K @ H  # [N*D, N*D]
            P_corrected = (A @ P[0] @ A.T + K @ R @ K.T)[None, :, :]  # [1, N*D, N*D]

            # Regular form update: can be unstable due to indefinite covariance
